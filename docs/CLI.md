...
一次提交**一批**改动（几十万条迁移用）。清单是 **JSONL**：一行一条改动，空行与 `#` 注释跳过；整份是 JSON 数组也认；`-` 表示从 stdin 读。

```text
{"op":"set","id":"<编号>","value":"新值"}       改值（值是字符串 / 数字 / 布尔 / null）
{"op":"set","id":"<编号>","ref":"<目标编号>"}    值 = 引用
{"op":"link","id":"<编号>","to":"<目标编号>"}    改引用（等价写法）
{"op":"rename","id":"<编号>","name":"新名字"}     改名
{"op":"rm","id":"<编号>"}                        删（置空）
{"op":"rm_subtree","id":"<编号>"}                连子树一起清空（每个后代都置空）
```

- **先全部校验、再落盘**：任一条不合法（编号不在这个文件里、模板定义受保护、名字超长…）**整批不动**，报错带条号。
- **一个进程、一次写入、一次台账登记**：不必为每条改动启动一次程序。同一个编号在一批里改多次，只写一条最终记录、只留一次痕。
- `--dry-run` 只校验：报「会改到几个节点」，一个字节都不写。
- `--no-history`：不留 `@history`（迁移推荐——不写的话每条改动会多两条留痕节点，文件与台账都更胖）。
- 只管**已有节点**；要批量**新增**用 `xr import --append`（整棵子树一次加）。
- **引用目标会被校验**：`link` / `ref` 的目标必须能在**本工作区（含其它分片）或本机目录**里找到，否则整批不动并报出第几条；确实要写「悬空引用」时加 `--allow-missing-target`。
- **文件没登记过也能跑**：复制到干净目录的词库天然没登记，命令会就地整份登记一次（单节点写命令也一样），不会把你卡在迁移第一步。
- **可以一次提交跨多个文件**：目标是**词库目录**时，按编号自动分派到它所在的分片，逐分片「先校验、再落盘」；输出会逐文件报账（哪个文件改了几条）。分片之间做不到整体事务，但会在失败信息里说清哪个成了、哪个没成，方便重跑。

**`rm` 与 `rm_subtree` 的取舍**（两者都是「置空」，不是物理删除——这是追加写的固有语义）：

- `rm` 只清空**这一个节点**的名字与值，它的孩子原样留着（于是会看到一个空壳挂着一堆内容）。
- `rm_subtree` 把**整棵子树**里每个节点都置空——语义上等于「这个义项连内容一起下架」。留痕打开时每个节点都会留下 `@history`，事后可以 `xr revert` 逐个还原。
- 想**原样留档、只是不进正文视图**（CiBase 的用法）：把子树顶部的节点 `rename` 成 `@` 开头的名字（如 `@已合并`），内容一个字节不动。

```bash
# 1) 先取出要改的编号（大文件要加 --force）
xr cat 大文件.xirang --force --ids | head -400000 | sed -n 's/.*<\([0-9a-f-]\{36\}\)>.*/\1/p' > ids.txt
# 2) 用脚本把 ids.txt 变成清单（一行一条），例如全部改成指向某个目标
# 3) 预演 → 真跑 → 收尾
xr batch 大文件.xirang 改动.jsonl --dry-run
xr batch 大文件.xirang 改动.jsonl --no-history
xr index compact          # 一次大批量之后，把台账日志并回块
```

> **大批量之前先看一眼台账日志**：日志偏大（比如超过 64 MB）时命令会提示一句——先跑 `xr index compact` 再批量会快很多（日志大本身不改变结果，只是定位要多读一点）。
