...
fn load(pol: &Policy, hooks: &dyn Hooks, file: &str) -> OpResult<(tree::Store, PathBuf)> {
    ...
    let store = tree::Store::load(&path).map_err(|e| load_error(&path, file, e))?;
...
pub fn validate(pol: &Policy, hooks: &dyn Hooks, file: &str) -> OpResult<ValidationOutcome> {
    let (store, _path) = load(pol, hooks, file)?;
    let errors = validator::validate(store.nodes())
        .into_iter()
...
pub fn create_node(
    ...
    let mut store = if path.exists() {
        let store = tree::Store::load(&path).map_err(OpError::internal)?;
        hooks.on_load(&path, &store);
        store
    } else {
        tree::Store::new()
...
pub fn template_define(
    ...
    // 与 `new` 一致：文件不存在就新建空库。
    let mut store = if path.exists() {
        let s = tree::Store::load(&path).map_err(OpError::internal)?;
...
pub fn import_data(
    ...
    let before = tree::Store::load(&path).ok().map(|s| s.len());
