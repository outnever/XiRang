...
ERROR_CODES = [
    ...
    _err("E002", "编号冲突", "同一个文件里出现两条相同编号的记录，且没有被声明为 append-v1 修订。", "根下挂 @protocol = append-v1 时，同编号多记录 = 修订，读时取最后一条，不报 E002。"),
    ...
    _err("F012", "尾部残片已忽略", "追加写到一半留下的半条记录：读取时忽略并提示，写入前截断到最后一个完整记录。"),
...
ERROR_HISTORY = [
    ...
    ("v1.1", "E002 收窄为「未被声明为修订的重复编号」；新增 F012 尾部残片已忽略。"),
]
...
PROTOCOL_TREE = [
    ...
    {"name": "已登记协议", "children": [
        ...
        {"name": "append-v1 单文件修订", "children": [
            {"name": "一句话", "value": "不拆文件也能「改一个词只写几十字节」——把改动作为一条同编号的新记录追加到文件末尾，读的时候每个编号取最后一条。"},
            {"name": "解决什么", "value": "息壤的节点是变长记录，原地改一个词就要挪动后面所有字节，等于整份重写（实测：179 MB / 347 万节点的文件，改一个词要 20.5 秒）；追加一条记录只要几十字节，毫秒级落盘。"},
            {"name": "怎么声明", "value": "被追加编辑过的那个根下挂一个辅助节点 @protocol = append-v1（随文件分发）。"},
            {"name": "怎么写", "value": "只往文件末尾追加一条「同一编号的新记录」（同编号 = 修订，后写覆盖）；新节点 = 新编号的一条记录。"},
            {"name": "怎么读", "value": "按文件内追加顺序，每个编号取最后一条（折叠视图）；同一编号的多条记录里只有最后一条算数。"},
            {"name": "怎么删", "value": "删 = 追加一条同编号、名字与值都为空的记录（留空槽位，编号不消失），与 shard-v1 一致。"},
            {"name": "重复编号算不算错", "value": "声明了 append-v1 的根下不算错（是修订）；没声明却出现重复编号，仍按 E002 编号冲突报。"},
            {"name": "合并", "value": "追加会越积越多；用 xr compact <文件> 把「同一编号只留最后一条」折叠回基版（手动，v1 不自动）。"},
            {"name": "写崩了怎么办", "value": "追加写到一半留下的半条记录 = 尾部残片：读取时忽略并提示（F012），写入前先把它截断。"},
            {"name": "代价", "value": "同一编号会短暂出现多份，文件比折叠版大；不 compact 就会一直涨。读取方必须实现「后写覆盖」，老读取器读这种文件会报 E002 或出错。"},
        ]},
