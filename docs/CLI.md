...
### `xr batch <文件> <清单文件|-> [--no-history] [--dry-run] [--json] [--yes]`
...
> **实测**（187 MB / 347 万节点，release）：**40 万条改引用 = 105 秒**（此前「一条一个命令」要按小时算），文件只增长约 27 MB；跑完 `xr validate` 0 错误、反向本 40 万条、`xr index compact` 4.4 秒。耗时约与「改动条数 + 涉及的祖先节点数」成正比；四十万条量级建议先 `--dry-run`，跑完补一次 `xr index compact`。
